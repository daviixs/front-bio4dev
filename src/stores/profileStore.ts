...
export const useProfileStore = create<ProfileState>((set, get) => ({
  ...
  updateLegenda: async (id: string, data: Partial<CreateLegendaDTO>) => {
    try {
      ...
      set({
        error: error.response?.data?.message || 'Erro ao atualizar legenda',
      });
    }
  },
  ...
  updatePage: async (id: string, data: Partial<CreatePageDTO>) => {
    try {
      ...
      const profile = get().profile;
      if (profile) {
        const pages = profile.pages?.map((p) =>
          p.id === id ? { ...p, ...data } : p,
        );
        set({ profile: { ...profile, pages } });
      }
      ...
      set({
        error: error.response?.data?.message || 'Erro ao atualizar página',
      });
    }
  },
  ...
  createSocial: async (data: CreateSocialDTO) => {
    try {
      ...
      set({
        error: error.response?.data?.message || 'Erro ao criar rede social',
      });
    }
  },
  ...
  updateSocial: async (id: string, data: Partial<CreateSocialDTO>) => {
    try {
      ...
      set({
        error: error.response?.data?.message || 'Erro ao atualizar rede social',
      });
    }
  },
  ...
  deleteSocial: async (id: string) => {
    try {
      ...
      set({
        error: error.response?.data?.message || 'Erro ao deletar rede social',
      });
    }
  },
  ...
  updateProjeto: async (id: string, data: Partial<CreateProjetoDTO>) => {
    try {
      ...
      const profile = get().profile;
      if (profile) {
        const projetos = profile.projetos?.map((p) =>
          p.id === id ? { ...p, ...data } : p,
        );
        set({ profile: { ...profile, projetos } });
      }
