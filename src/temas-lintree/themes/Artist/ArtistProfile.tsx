      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center">
        ...
        <div className="w-full flex flex-col gap-4 mt-12">
          {links.map((link, i) => (
            <button
              key={i}
              className="w-full border border-zinc-700 hover:border-purple-500 hover:bg-zinc-800 transition-all py-4 px-6 rounded-full flex justify-between items-center group"
            >
              ...
              <Disc
                size={16}
                className="text-zinc-600 group-hover:text-purple-500 group-hover:rotate-180 transition-all duration-500"
              />
            </button>
          ))}
        </div>
