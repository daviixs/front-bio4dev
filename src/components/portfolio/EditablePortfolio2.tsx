...
import { footerApi, legendaApi, techStackApi, workExperienceApi, projetosApi } from "@/lib/api"; // APIs
import type {
  ProfileComplete,
  Social,
  Projeto,
...
const SocialCard: React.FC<SocialCardProps> = ({ item, onEdit, isDemo }) => {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start z-10">
        ...
        >
             {onEdit ? <Pencil size={16} className="text-white" /> : <ArrowUpRight size={18} className="text-white" />}
        </div>
      </div>
      ...
      <div className="z-10">
        {item.id === "github" && (
         ...
         {!item.id && isDemo && (
             <span className="mb-2 inline-block px-2.5 py-0.5 text-[10px] font-bold bg-yellow-500/20 text-yellow-500 rounded uppercase tracking-wider">
             Demo
           </span>
         )}
...
const ExperienceTimeline: React.FC<ExperienceTimelineProps> = ({ data, onEdit, onAdd }) => {
  return (
    <div className="bg-[#121318] rounded-[2rem] p-6 border border-white/5">
      <div className="flex justify-between items-center mb-6">
        ...
        {onAdd && (
            <Button onClick={onAdd} size="sm" variant="ghost" className="text-yellow-500 hover:text-yellow-400 hover:bg-yellow-500/10">
      ...
      <div className="relative border-l border-gray-800 ml-3 space-y-8">
        {(!data || data.length === 0) && <p className="text-gray-500 text-sm ml-6">No experience added.</p>}
        {data && data.map((item, index) => (
          <div key={item.id} className="ml-6 relative group cursor-pointer" onClick={() => onEdit && onEdit(item)}>
            {/* Dot on timeline */}
            <span
      ...
      <DialogContent className="sm:max-w-md bg-[#18181b] border-gray-800 text-white">
        ...
        <DialogFooter className="flex justify-end">
          <div className="flex gap-2">
            ...
            <Button onClick={handleConfirm} className="bg-yellow-500 text-black hover:bg-yellow-600">
              Adicionar
            </Button>
          </div>
        </DialogFooter>
  ...
  return (
    <div className="bg-[#121318] rounded-[2rem] p-6 border border-white/5">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {(!data || data.length === 0) && <span className="text-gray-500 text-sm">No technologies added yet.</span>}
        {data && data.map((tech) => (
          <span
          ...
          >
            ...
            {onRemove && (
                <button onClick={() => onRemove(tech)} className="hover:text-red-500 hidden group-hover:inline-block">
                    <X size={14} />
                </button>
            )}
          </span>
      ...
      {onAdd && (
          <>
            <Button size="sm" onClick={() => setIsDialogOpen(true)} className="bg-yellow-500 text-black hover:bg-yellow-600 h-9 flex items-center gap-2">
                <Plus size={16} />
...
const SOCIAL_OPTIONS = [
  { id: 'dribbble', name: 'Dribbble', icon: Dribbble, colorClass: 'bg-[#ea4c89] hover:bg-[#ff5da0]', url: 'https://dribbble.com/' }, 
  { id: 'behance', name: 'Behance', icon: Linkedin, colorClass: 'bg-[#1769ff] hover:bg-[#4080ff]', url: 'https://behance.net/' }, 
  { id: 'pinterest', name: 'Pinterest', icon: Pin, colorClass: 'bg-[#e60023] hover:bg-[#ff1a3c]', url: 'https://pinterest.com/' },
  { id: 'github', name: 'GitHub', icon: Github, colorClass: 'bg-[#18181b] hover:bg-[#27272a] border border-gray-800', url: 'https://github.com/' },
  { id: 'gitlab', name: 'GitLab', icon: Gitlab, colorClass: 'bg-[#fc6d26] hover:bg-[#fd8c52]', url: 'https://gitlab.com/' },
  { id: 'bitbucket', name: 'Bitbucket', icon: FolderGit2, colorClass: 'bg-[#0052cc] hover:bg-[#2684ff]', url: 'https://bitbucket.org/' },
  { id: 'stackoverflow', name: 'StackOverflow', icon: Code2, colorClass: 'bg-[#f48024] hover:bg-[#ff9a4d]', url: 'https://stackoverflow.com/' },
  { id: 'codepen', name: 'CodePen', icon: Codepen, colorClass: 'bg-black hover:bg-gray-800 border border-gray-800', url: 'https://codepen.io/' },
  { id: 'dev', name: 'DEV.to', icon: Code2, colorClass: 'bg-white hover:bg-gray-100', textColorClass: 'text-black', url: 'https://dev.to/' },
  { id: 'linkedin', name: 'LinkedIn', icon: Linkedin, colorClass: 'bg-[#0077b5] hover:bg-[#006399]', url: 'https://linkedin.com/in/' },
  { id: 'medium', name: 'Medium', icon: Mail, colorClass: 'bg-white hover:bg-gray-100', textColorClass: 'text-black', url: 'https://medium.com/' },
  { id: 'youtube', name: 'YouTube', icon: Youtube, colorClass: 'bg-[#FF0000] hover:bg-[#ff3333]', url: 'https://youtube.com/' },
  { id: 'instagram', name: 'Instagram', icon: Instagram, colorClass: 'bg-gradient-to-br from-purple-600 to-pink-500 hover:from-purple-700 hover:to-pink-600', url: 'https://instagram.com/' },
  { id: 'facebook', name: 'Facebook', icon: Facebook, colorClass: 'bg-[#1877F2] hover:bg-[#3b87f4]', url: 'https://facebook.com/' },
  { id: 'twitter', name: 'Twitter', icon: Twitter, colorClass: 'bg-[#1DA1F2] hover:bg-[#4cb5f5]', url: 'https://twitter.com/' },
  { id: 'discord', name: 'Discord', icon: MessageCircle, colorClass: 'bg-[#5865F2] hover:bg-[#7983f5]', url: 'https://discord.com/' },
  { id: 'whatsapp', name: 'WhatsApp', icon: Phone, colorClass: 'bg-[#25D366] hover:bg-[#4ce285]', url: 'https://wa.me/' },
  { id: 'telegram', name: 'Telegram', icon: Send, colorClass: 'bg-[#0088cc] hover:bg-[#2cabeb]', url: 'https://t.me/' },
  { id: 'email', name: 'Email', icon: Mail, colorClass: 'bg-[#1e293b] hover:bg-[#263345]', url: 'mailto:' },
];
...
export function EditablePortfolio2({ profile }: EditablePortfolio2Props) {
  ...
  const hasUserSocials = mappedSocials.length > 0;
  const socials = hasUserSocials ? mappedSocials : DEMO_SOCIAL_LINKS;
  ...
  const mappedProjects: Project[] = (currentProfile.projetos || []).map((p) => ({
    id: p.id,
    title: p.nome,
  ...
  const handleResumeUpdate = async (resumeUrl: string) => {
    ...
    try {
      await footerApi.update(currentProfile.footer.id, { ...currentProfile.footer, resumeUrl });
      setCurrentProfile({ ...currentProfile, footer: { ...currentProfile.footer, resumeUrl } });
  ...
  const saveSocialEdit = () => {
      ...
      const existingIndex = newSocials.findIndex(s => s.plataforma.toLowerCase() === editingSocial.name.toLowerCase() || s.id.toString() === editingSocial.id);

      if (existingIndex >= 0) {
          newSocials[existingIndex] = { ...newSocials[existingIndex], url: editUrl };
      } else {
          newSocials.push({
              id: Date.now().toString(),
              plataforma: editingSocial.name as any,
              url: editUrl,
  ...
  const deleteSocial = () => {
       if (!editingSocial) return;
       const newSocials = (currentProfile.social || []).filter(s => s.plataforma.toLowerCase() !== editingSocial.name.toLowerCase() && s.id.toString() !== editingSocial.id);
       setCurrentProfile({ ...currentProfile, social: newSocials });
       setIsEditModalOpen(false);
  ...
  const handleAddTech = async (newTech: Technology) => {
    ...
    setCurrentProfile({ ...currentProfile, techStack: updatedTechStack as any });
  ...
  const [expForm, setExpForm] = useState({ role: '', company: '', date: '', description: '', current: false });

  /* EXP HANDLERS */
  const openExpAdd = () => {
      setEditingExp(null);
      setExpForm({ role: '', company: '', date: '', description: '', current: false });
  ...
  const handleSaveExp = async () => {
       const newWork = {
           ...
           position: expForm.role || "Role", 
           period: expForm.date || "Date",
           summary: expForm.description || "",
       ...
       try {
           if (editingExp) {
               ...
               await workExperienceApi.create({ ...newWork, profileId: currentProfile.id });
           ...
           const newBackendWork = {
               id: newExpUI.id,
               company: newExpUI.company, // Assuming we fixed mapping (see below) or this will just be consistent with itself
               position: newExpUI.role,
               period: newExpUI.date,
               summary: newExpUI.description,
           ...
           let newList;
           if(editingExp) {
               newList = currentList.map(item => item.id === editingExp.id ? { ...item, ...newBackendWork } : item);
           } else {
               newList = [...currentList, { ...newBackendWork, profileId: currentProfile.id }];
           }
  ...
  const handleDeleteExp = async () => {
      ...
      try {
          ...
          const newList = (currentProfile.workHistory || []).filter(i => i.id !== editingExp.id);
  ...
  const openProjAdd = () => {
      setEditingProj(null);
      setProjForm({ title: '', description: '', githubUrl: '', deployUrl: '', thumbnail: '', technologies: '' });
  ...
  const openProjEdit = (projUI: Project) => {
      ...
      const original = (currentProfile.projetos || []).find(p => p.id === projUI.id);
  ...
  const handleSaveProj = async () => {
      ...
      const payload = {
          ...
          codeLink: projForm.githubUrl,
          demoLink: projForm.deployUrl,
          gif: projForm.thumbnail, 
          // tecnologias: projForm.technologies.split(',').map(s=>s.trim()).filter(Boolean) 
      };
      ...
      try {
          if (editingProj) {
               ...
               await projetosApi.create({ ...payload, profileId: currentProfile.id });
          ...
          // Local State Update
           ...
           let newProjsList;
           if(editingProj) {
               newProjsList = currentProjs.map(p => p.id === editingProj.id ? { ...p, ...newProjUI } : p);
           } else {
  ...
  const handleDeleteProj = async () => {
    ...
    try {
        ...
        const newProjsList = (currentProfile.projetos || []).filter(p => p.id !== editingProj.id);
        setCurrentProfile({ ...currentProfile, projetos: newProjsList });
        setIsProjModalOpen(false);
            ...
            <DialogContent className="sm:max-w-md bg-[#18181b] border-gray-800 text-white">
                ...
                <div className="flex flex-col gap-4 py-4 max-h-[70vh] overflow-y-auto">
                    <div className="flex flex-col gap-2">
                        <Label>Title</Label>
                        <Input value={projForm.title} onChange={e => setProjForm({...projForm, title: e.target.value})} placeholder="Project Title" className="bg-black/50 border-gray-700"/>
                    </div>
                     <div className="flex flex-col gap-2">
                        <Label>Short Description</Label>
                        <Textarea value={projForm.description} onChange={e => setProjForm({...projForm, description: e.target.value})} placeholder="What does it do?" className="bg-black/50 border-gray-700" rows={3}/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Github URL</Label>
                        <Input value={projForm.githubUrl} onChange={e => setProjForm({...projForm, githubUrl: e.target.value})} placeholder="https://github.com/..." className="bg-black/50 border-gray-700"/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Live URL (Optional)</Label>
                        <Input value={projForm.deployUrl} onChange={e => setProjForm({...projForm, deployUrl: e.target.value})} placeholder="https://..." className="bg-black/50 border-gray-700"/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Technologies (Comma separated)</Label>
                        <Input value={projForm.technologies} onChange={e => setProjForm({...projForm, technologies: e.target.value})} placeholder="React, Node, TypeScript" className="bg-black/50 border-gray-700"/>
                    </div>
                </div>
                 <DialogFooter className="flex justify-between w-full">
                     {editingProj ? (
                         <Button variant="destructive" onClick={handleDeleteProj} size="sm">
                             <Trash2 size={16} className="mr-2"/> Remove
                         </Button>
                     ) : <div></div>}
                     <div className="flex gap-2">
                        <Button variant="ghost" onClick={() => setIsProjModalOpen(false)} className="text-gray-400">Cancel</Button>
                        <Button onClick={handleSaveProj} className="bg-yellow-500 text-black hover:bg-yellow-600">Save</Button>
                     </div>
                </DialogFooter>
            </DialogContent>
        </Dialog>
            ...
            <DialogContent className="sm:max-w-md bg-[#18181b] border-gray-800 text-white">
                ...
                <div className="flex flex-col gap-4 py-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <Input value={expForm.role} onChange={e => setExpForm({...expForm, role: e.target.value})} placeholder="e.g. Senior Developer" className="bg-black/50 border-gray-700"/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Company</Label>
                        <Input value={expForm.company} onChange={e => setExpForm({...expForm, company: e.target.value})} placeholder="e.g. Google" className="bg-black/50 border-gray-700"/>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Period</Label>
                        <Input value={expForm.date} onChange={e => setExpForm({...expForm, date: e.target.value})} placeholder="e.g. 2023 - Present" className="bg-black/50 border-gray-700"/>
                    </div>
                    <div className="flex flex-col gap-2">
                         <div className="flex items-center gap-2">
                            <Checkbox id="current" checked={expForm.current} onCheckedChange={(c) => setExpForm({...expForm, current: c as boolean})} />
                            <Label htmlFor="current">I currently work here</Label>
                        </div>
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label>Description</Label>
                        <Textarea value={expForm.description} onChange={e => setExpForm({...expForm, description: e.target.value})} className="bg-black/50 border-gray-700" rows={4}/>
                    </div>
                </div>
                 <DialogFooter className="flex justify-between w-full">
                     {editingExp ? (
                     ...
                     <div className="flex gap-2">
                        <Button variant="ghost" onClick={() => setIsExpModalOpen(false)} className="text-gray-400">Cancel</Button>
                        <Button onClick={handleSaveExp} className="bg-yellow-500 text-black hover:bg-yellow-600">Save</Button>
                     </div>
                </DialogFooter>
            </DialogContent>
        </Dialog>
            ...
            <DialogContent className="sm:max-w-md bg-[#18181b] border-gray-800 text-white">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        {editingSocial && <editingSocial.icon size={20} className="text-yellow-500"/>}
                ...
                <DialogFooter className="flex justify-between sm:justify-between w-full">
                     {editingSocial && currentProfile.social?.some(s => s.plataforma === editingSocial.name || s.id.toString() === editingSocial.id) ? (
                     ...
                     <div className="flex gap-2">
                        <Button variant="ghost" onClick={() => setIsEditModalOpen(false)} className="text-gray-400">Cancel</Button>
                        <Button onClick={saveSocialEdit} className="bg-yellow-500 text-black hover:bg-yellow-600">Save</Button>
                     </div>
                </DialogFooter>
            </DialogContent>
        </Dialog>
            ...
            <main className="w-full max-w-md md:max-w-xl lg:max-w-2xl z-10 px-6 py-10 flex flex-col gap-6">
                ...
                <div className="bg-[#121318] rounded-[2rem] p-6 sm:p-8 border border-white/5 shadow-sm">
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                        ...
                        <Avatar
                            src={legenda?.legendaFoto || currentProfile.avatarUrl || "https://api.dicebear.com/7.x/avataaars/svg?seed=Default&backgroundColor=fbbf24"}
                        ...
                        <div className="flex-1 text-center sm:text-left mt-2">
                            ...
                            <div className="flex flex-col gap-2 mb-4">
                                <div className="flex items-center gap-2 text-gray-400">
                                    <User size={16} />
                                    <EditableField
                                        value={legenda?.subtitulo || "Building digital products that matter."}
                                ...
                                <div className="flex items-center gap-2 text-gray-400">
                                    <MapPin size={16} />
                                    <EditableField
                                        value={currentProfile.footer?.email || "Location/Contact"}
                                        onSave={(val) => handleFooterUpdate('email', val)}
                                        className="text-sm hover:bg-white/5 p-1 rounded"
                                    />
                                </div>
                            </div>
                ...
                <section className="grid grid-cols-2 gap-4">
                    {socials.map((link, index) => (
                        <SocialCard 
                            ...
                            isDemo={!hasUserSocials}
                        />
                    ))}
                </section>
                
                <div className="bg-[#121318] rounded-[2rem] p-6 border border-white/5">
                    <p className="text-sm text-gray-400 mb-3 font-medium">Add more profiles:</p>
                    <div className="flex flex-wrap gap-2">
                        {SOCIAL_OPTIONS.map((option) => (
                            <button
                                ...
                                className={`
                                    ...
                                    ${option.colorClass.replace("bg-", "hover:bg-")} 
                                `}
                            >
                                <option.icon size={16} className="text-gray-300 group-hover:text-white" />
                                <span className="text-xs font-medium text-gray-300 group-hover:text-white">{option.name}</span>
                ...
                <section>
                    <div className="flex justify-between items-center mb-4 px-2">
                        ...
                        <Button onClick={openProjAdd} size="sm" variant="ghost" className="text-yellow-500 hover:text-yellow-400 hover:bg-yellow-500/10">
                            <Plus size={16} className="mr-1"/> Add
                        </Button>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {(!projectsData || projectsData.length === 0) && <p className="text-gray-500 text-sm ml-2">No projects added.</p>}
                        {projectsData.map((project) => (
                            <div key={project.id} className="relative group">
                                <ProjectCard project={project} />
                                <button 
                                    onClick={(e) => { e.stopPropagation(); openProjEdit(project); }}
