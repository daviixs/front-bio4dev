...
interface ActivistEditorProps {
  profileData: ProfileData;
  setProfileData: (data: ProfileData) => void;
}

const ActivistEditor: React.FC<ActivistEditorProps> = ({
  profileData,
  setProfileData,
}) => {
  ...
  const addButton = () => {
    const newButton: ProfileButton = { label: "Nova Ação", url: "" };
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="bg-gradient-to-r from-emerald-50 to-green-50 p-6 rounded-xl border-2 border-emerald-200">
        ...
        <p className="text-emerald-700">
          Personalize seu perfil de ativismo e causas sociais
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            placeholder="Prof. Gabriel Sustentável"
            className="border-emerald-200 focus:border-emerald-500"
          />
        </div>
        <div>
          <Label htmlFor="bio" className="text-emerald-700">
            Bio / Missão
          </Label>
          <Textarea
            id="bio"
            ...
            placeholder="Educador ambiental. Vamos transformar nossa comunidade juntos. 🌱✊"
            rows={3}
            className="border-emerald-200 focus:border-emerald-500"
          />
        </div>
        <div>
          <Label htmlFor="photoUrl" className="text-emerald-700">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        {profileData.buttons.map((button, index) => (
          <div
            key={index}
            className="flex gap-2 p-4 bg-emerald-50 rounded-lg border border-emerald-100"
          >
            <div className="flex-1 space-y-2">
              <Input
                value={button.label}
                onChange={(e) => updateButton(index, "label", e.target.value)}
                placeholder="Curso Grátis: Horta Urbana"
