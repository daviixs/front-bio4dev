import type { ReactNode } from 'react';

export interface GoogleAuthGateChrome {
  modalBackdrop: string;
  modalCard: string;
  modalAccentLine: string;
  modalEyebrow: string;
  modalTitle: string;
  modalBody: string;
  modalError: string;
  modalContinue: string;
  modalClose: string;
}

interface GoogleAuthGateProps {
  open: boolean;
  isLoading: boolean;
  error: string | null;
  onClose: () => void;
  onContinue: () => Promise<void> | void;
  chrome: GoogleAuthGateChrome;
  eyebrow: ReactNode;
  title: ReactNode;
  body: ReactNode;
  continueLabel?: ReactNode;
  loadingLabel?: ReactNode;
  closeLabel?: ReactNode;
  titleId?: string;
}

const GoogleGlyph = () => (
  <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      fill="#FBBC05"
    />
    <path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
  </svg>
);

export function GoogleAuthGate({
  open,
  isLoading,
  error,
  onClose,
  onContinue,
  chrome,
  eyebrow,
  title,
  body,
  continueLabel = 'Continuar com Google',
  loadingLabel = 'Conectando com Google...',
  closeLabel = 'Continuar editando',
  titleId = 'google-auth-gate-title',
}: GoogleAuthGateProps) {
  if (!open) return null;

  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center px-4 py-6 ${chrome.modalBackdrop}`}
      role="dialog"
      aria-modal="true"
      aria-labelledby={titleId}
    >
      <div
        className={`relative w-full max-w-md overflow-hidden rounded-[28px] p-6 ${chrome.modalCard}`}
      >
        <div
          className={`pointer-events-none absolute inset-x-6 top-0 h-px ${chrome.modalAccentLine}`}
        />

        <div className="space-y-5">
          <div className="space-y-3">
            <p
              className={`text-xs uppercase tracking-[0.3em] ${chrome.modalEyebrow}`}
            >
              {eyebrow}
            </p>
            <h2
              id={titleId}
              className={`text-2xl font-semibold tracking-tight ${chrome.modalTitle}`}
            >
              {title}
            </h2>
            <p className={`text-sm leading-6 ${chrome.modalBody}`}>{body}</p>
          </div>

          {error ? <div className={chrome.modalError}>{error}</div> : null}

          <button
            type="button"
            onClick={() => {
              void onContinue();
            }}
            disabled={isLoading}
            className={chrome.modalContinue}
          >
            <GoogleGlyph />
            {isLoading ? loadingLabel : continueLabel}
          </button>

          <button
            type="button"
            onClick={onClose}
            disabled={isLoading}
            className={chrome.modalClose}
          >
            {closeLabel}
          </button>
        </div>
      </div>
    </div>
  );
}
